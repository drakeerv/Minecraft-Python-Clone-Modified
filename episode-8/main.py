...
class Window(pyglet.window.Window):
	...
	def on_draw(self):
		...
		gl.glClearColor(0.44, 0.77, 0.99, 1.0)
		self.clear()
		self.world.draw(self.camera.position)
...
class Game:
	def __init__(self):
		self.config = gl.Config(depth_size = 16)
		self.window = Window(config = self.config, width = int(optionsWidth), height = int(optionsHeight), caption = "Minecraft clone", resizable = True, vsync = bool(optionsVsync))
