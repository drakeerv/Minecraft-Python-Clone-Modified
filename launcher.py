...
required_modules = ['pyglet', 'requests']
needed_modules = []
...
for i in required_modules:
    try:
        ...
        needed_modules.append(i)

if len(needed_modules) > 0:
    call(f"pip install {' '.join(needed_modules)}", shell=True)

import requests

#Get all the versions
versions = requests.get("https://roundedsimilarpixels.drakeerv.repl.co/versions.txt").content.decode().split("\n")
...
if not os.path.isdir("./textures"):
    ...
    download_url(f"https://roundedsimilarpixels.drakeerv.repl.co/{file_name}", f"./{file_name}")
...
def launch():
    ...
    with contextlib.suppress(TimeoutExpired):
        call(f"python main.py", shell=True, timeout=5)
...
def create_options():
    ...
    canvas = tk.Canvas(options, width = 500, height = 400, bg = 'gray95', relief = 'raised')
    canvas.pack()
    ...
    entry_height = tk.Spinbox(options, from_=1, to=9000)
    entry_width = tk.Spinbox(options, from_=1, to=9000)
...
def create_installation_menu():
    ...
    canvas = tk.Canvas(installation_menu, width = 500, height = 400, bg = 'gray95', relief = 'raised')
    canvas.pack()

    #Download the selected version
    def download_version():
        download_url(f"https://roundedsimilarpixels.drakeerv.repl.co/{versions_download_var.get()}", f"./{versions_download_var.get()}")
    ...
    versions_download_var.set(versions[-1])
    versions_download_picker = tk.OptionMenu(installation_menu, versions_download_var, *versions)
    ...
    versions_download_label = tk.Label(installation_menu, text="Please pick the veriosn you want to install:")
...
needed_modules_label = tk.Label(root, text=f"Please install these modules: {', '.join(needed_modules)}")
...
options_button = tk.Button(root, text="Options", command=create_options, bg='blue', fg='white', font=('helvetica', 12, 'bold'))
launch_version = tk.Label(root, text=f"Launcher version: 1")
...
if len(needed_modules) > 0:
    needed_modules_label.place(relx=0.5, rely=0.4, anchor=tk.CENTER)
